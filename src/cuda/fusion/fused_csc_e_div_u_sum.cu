...
#include "../utils.h"
#include "fused_csc_e_div_u_sum.h"
#include "../macro.h"
...
__global__ void EDivUSumKernel(
  ...
  while (ty < num_cols) {
    ...
    while (tx < out_len) {
      ...
      for (Idx i = indptr[ty]; i < indptr[ty + 1]; ++i) {
        ...
        DType value = BinaryOp::Call(eoff + rhs_add,uoff + lhs_add);
...
template <typename Idx, typename DType, typename BinaryOp, typename ReduceOp>
void  EDivUSumCUDA(const BcastOff& bcast, const std::shared_ptr<CSC> csc,
...
//     SWITCH_IDX(use_idx, {
      CUDA_KERNEL_CALL(
          (EDivUSumKernel<Idx, DType, BinaryOp, ReduceOp, false, false>),
...
void EDivUSumCSC(const std::string& op, const std::string& reduce,
  ...
  if (reduce == "sum") {
    ID_TYPE_SWITCH(csc->indices.scalar_type(), IdType, {
      typedef float DType;
      SWITCH_OP(op, Op, {
        EDivUSumCUDA<IdType, DType, Op, impl::reduce::Sum<IdType, DType> >(
