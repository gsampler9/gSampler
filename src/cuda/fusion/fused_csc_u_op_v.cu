...
template <typename Idx, typename DType, typename BinaryOp,
          ...
          int RhsTarget = 2>
__global__ void FUSED_CSC_U_OP_V_Kernel(
    ...
    const Idx* __restrict__ indices, int64_t N, int64_t E,
  ...
  while (ty < E) {
    const Idx src = gs::impl::_ldg(indices + ty);
...
void FUSED_CSC_U_OP_V_CUDA(const BcastOff& bcast,
...
torch::Tensor lhs1,
  ...
  int64_t reduce_dim =  bcast.reduce_size;
    ...
    BCAST_IDX_CTX_SWITCH(bcast, false, lhs_off, rhs_off, {
      CUDA_KERNEL_CALL((FUSED_CSC_U_OP_V_Kernel<Idx, DType, Op>),
                       nblks, nthrs, lhs_data1, rhs_data1, out_data1, lhs_data2, rhs_data2, out_data2,indptr, indices,
                        N,E, reduce_dim, lhs_off, rhs_off, lhs_len,
                       rhs_len, len);
    });
}
void FUSED_CSC_U_OP_V(const std::string& op, const BcastOff& bcast,
  ...
  ID_TYPE_SWITCH(csc->indices.scalar_type(), IdType, {         
    FLOAT_TYPE_SWITCH(out1.scalar_type(), DType, {
      SWITCH_OP(op, Op, {   
      FUSED_CSC_U_OP_V_CUDA<IdType, DType, Op>(bcast, csc,lhs1,rhs1, out1,lhs2,rhs2, out2);
              });
          });
    });
}
